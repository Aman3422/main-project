...
'''
It contains compensation business logics
'''
...
class CompensationService(BaseService):
    ...
    def search(self, params):
        ...
        columnName = ('id', 'staffMember', 'paymentAmount', 'dateApplied', 'state','stid','sid')
        res = {
            "data": [],
            "MaxId": 1,
        }
    ...
    def search1(self, params):
        ...
        print("-------pageNo-->>", pageNo)
        sql = "select * from sos_compensation where 1!=1"
        val1 = params.get("paymentAmount", None)
        val2 = params.get("dateApplied", None)
        val3 = params.get("stid", None)
        val4 = params.get("sid", None)

        print("-----val-->>", val1)
        if DataValidator.isNotNull(val1):
            sql += " or paymentAmount = '" + str(val1) + "'"
        if DataValidator.isNotNull(val2):
            sql += " or dateApplied like '" + str(val2) + "%%' "
        if DataValidator.isNotNull(val3):
            sql += " or stid = '" + str(val3) + "' "
