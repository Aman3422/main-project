...
#         cursor.execute(sql, [pageNo, self.pageSize])
#         result = cursor.fetchall()
#         columnfullName = ('id','fullName','username','password','dob','number')
#         res = {
#             "data": [],
#             "MaxId": 1,
#         }
...
#         for x in result:
#             params['MaxId'] = x[0]
#             res['data'].append({columnfullName[i]: x[i] for i, _ in enumerate(x)})
#         print("MMMMMMMMMM", params.get("MaxId"))
#         return res
#
#     def search1(self, params):
...
#         count = 0
#         for x in result:
#             # print("--------with column-->>",{columnfullName[i] :  x[i] for i, _ in enumerate(x)})
#             params['MaxId'] = x[0]
#             print("-------params['MaxId']-->>", params['MaxId'])
#             res["data"].append({columnfullName[i]: x[i] for i, _ in enumerate(x)})
#         return res
