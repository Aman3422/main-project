...
class OrderCtl(BaseCtl):
    ...
    def preload(self, request, params={}):
        preloadList = [
            ...
            {'pid': 2, 'product': "computer"},
            {'pid': 3, 'product': "Laptop"},
            {'pid': 4, 'product': "Watch"},
            {'pid': 5, 'product': "Tablet"},
            {'pid': 6, 'product': "Sports"}
        ]
    ...
    def input_validation(self):
        ...
        if (DataValidator.isNull(self.form["quantity"])):
            ...
            inputError['quantity'] = "Incorrect quantity,Id should be number"
            self.form['error'] = True
        else:
        ...
        if (DataValidator.isNull(self.form['pid'])):
            self.form["error"] = True
            inputError["pid"] = "category can not be null"
        ...
        if (DataValidator.isNull(self.form["amount"])):
            ...
            inputError['amount'] = "Incorrect amount,Id should be number"
            self.form['error'] = True
        else:
    ...
    def input_validation1(self):
        ...
        if (DataValidator.isNotNull(self.form["quantity"])):
            if (DataValidator.max_len_20(self.form['quantity'])):
                ...
                inputError['quantity'] = "Incorrect quantity,amount should be number"
                self.form['error'] = True
            else:
                if (DataValidator.is_0(self.form['quantity'])):
                    inputError['quantity'] = "quantity can not be 0 or less than 0"
        ...
        if (DataValidator.isNotNull(self.form["amount"])):
            if (DataValidator.max_len_20(self.form['amount'])):
                ...
                inputError['amount'] = "Incorrect amount,amount should be number"
                self.form['error'] = True
            else:
                if (DataValidator.is_0(self.form['amount'])):
                    inputError['amount'] = "amount can not be 0 or less than 0"
    ...
    def search1(self, request, params={}):
        ...
        if (json_request):
            ...
            params["version"] = json_request.get("amount", None)
            params["pageNo"] = json_request.get("pageNo", None)
    ...
    def save(self, request, params={}):
        ...
        if (self.input_validation()):
            ...
            if (self.form["id"] > 0):
                dup = Order.objects.exclude(id=self.form['id']).filter(pid=self.form["pid"])
                if (dup.count() > 0):
                    res["error"] = True
                    res["message"] = "Product Name already exists"
                else:
                ...
                duplicate = Order.objects.filter(pid=self.form["pid"])
